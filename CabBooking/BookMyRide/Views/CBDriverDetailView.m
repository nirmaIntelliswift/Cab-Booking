...
-(void)addAllViews{
    ...
    _lblDriverCarNumber.text = @"";
    _lblDriverName.text = @"";
    _lblDriverCarType.text = @"";
    _lblRateValue.text = @"";
    
    [_imgDriverPhoto setImageFromUrl:nil];
}
-(void)addTopSeprator{
...
-(void)addPhoto{
    _imgDriverPhoto = [CBUIImageView new];
    ...
    _imgDriverPhoto.clipsToBounds = YES;
}
-(void)addRateImage{
    UIImage *rateImage = [UIImage imageNamed:@"ic_favorite"];
    _imgRate = [CBUIImageView new];
...
-(void)setData:(id)data{
    CBCarModel *cabDetail = (CBCarModel*)data;
    _lblDriverCarNumber.text = cabDetail.cabRegNo;
    _lblDriverName.text = cabDetail.driverName;
    _lblDriverCarType.text = cabDetail.cabModel;
    _lblRateValue.text = [NSString stringWithFormat:@"%.1f",cabDetail.cabRating];
   
    [_imgDriverPhoto setImageFromUrl:cabDetail.driverImage];
}
-(void)unselectAllPymentMode{
      _btnPaymentCash.imgRight.image = [UIImage imageNamed:@"unselect-checkbox"];
     _btnPaymentMyWallet.imgRight.image = [UIImage imageNamed:@"unselect-checkbox"];
}
-(void)selectCash:(BOOL)selected{
    [self unselectAllPymentMode];
    if (selected) {
        _btnPaymentCash.imgRight.image = [UIImage imageNamed:@"select-checkbox"];
    }
    else{
         _btnPaymentCash.imgRight.image = [UIImage imageNamed:@"unselect-checkbox"];
    }
...
-(void)selectMyWallet:(BOOL)selected{
    [self unselectAllPymentMode];
    if (selected) {
        _btnPaymentMyWallet.imgRight.image = [UIImage imageNamed:@"select-checkbox"];
    }
    else{
        _btnPaymentMyWallet.imgRight.image = [UIImage imageNamed:@"unselect-checkbox"];
    }
