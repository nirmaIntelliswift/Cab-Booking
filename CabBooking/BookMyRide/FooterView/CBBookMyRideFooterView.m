...
#define kCHOOSE_DESTINATION  @"Choose destination"
...
-(BOOL)isDestinationChosen{
    if([_btnDestination.lblTitle.text isEqualToString:kCHOOSE_DESTINATION]){
        return NO;
    }
    return YES;
...
-(void)addCurrentLocationView{
    ...
    CBUIImageView *dividerLine = [[CBUIImageView alloc]initWithImage:dividerImage];
...
-(CGFloat)setRideNowView{
    [self hideAllViews];
    _btnCurrentLocation.userInteractionEnabled = YES;
...
-(CGFloat)setConfirmBookingView{
    ...
    _btnCurrentLocation.userInteractionEnabled = NO;
    _btnDestination.userInteractionEnabled = YES;
...
-(CGFloat)setCancelBookingView{
    ...
    _btnDestination.userInteractionEnabled = NO;
...
-(void)selectMyWallet:(BOOL)selected{
    [_viewDriverDetail selectMyWallet:selected];
}
-(void)selectCash:(BOOL)selected{
    [_viewDriverDetail selectCash:selected];
}
#pragma mark fill Server Details

-(void)fillCarCategoryData:(CBCarCategoryModel*)carCategory{
    [_viewCarDetail fillCarCategoryData:carCategory];
}
-(void)fillRideEstimationTime:(CBRideEstimationModel*)rideEstimation{
     [_viewCarDetail fillRideEstimationData:rideEstimation];
}
-(void)fillCarEstimateFare:(CBRideFareModel*)rideFare{
    [_viewCarDetail setEstimatedFare:rideFare.fare];
}
-(void)fillRideConfirmationDetails:(CBRideConfirmationModel*)rideConfirmation{
    [_viewDriverDetail setData:rideConfirmation.bookedCab];
}
@end
